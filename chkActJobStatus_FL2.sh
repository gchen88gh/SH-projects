...
#
#
  local FUNC JOBNAME=$2
  echo "$1" | grep -iq run && FUNC=startJob
  echo "$1" | grep -iq stop && FUNC=stopJob
  [ -z "$FUNC" ] && FUNC=getJobStatus && return 1
  /usr/bin/curl -s "http://${SERVER}/jmx-console/HtmlAdaptor?action=invokeOpByName&name=accounttex2%3Aname%3DJobManager&methodName=$FUNC&argType=java.lang.String&arg0=$JOBNAME" | grep -q "Operation completed successfully without a return value"
#
}
...
creditMemoRetrievalJob: Running
